...
adl_lnd_sink = "{adl_path}{lnd_path}".format(adl_path=var_adls_uri, lnd_path=lnd_path)
...
for lnd_year_month_path in sorted(cf.list_subdirectory(dbutils, lnd_path), key=lambda p: int(p.split('/')[-1])):
  df_adl_files = cf.list_adl_files(spark, dbutils, lnd_year_month_path)
  
  year_month = lnd_year_month_path.split('/')[-1]
  files_folder = cf.list_subdirectory(dbutils, lnd_year_month_path)
