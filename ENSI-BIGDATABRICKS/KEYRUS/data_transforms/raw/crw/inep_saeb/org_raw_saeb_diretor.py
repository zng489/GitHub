...
#   'schema': 'inep_saeb',
#   'table': 'diretor',
#   'path_destination': 'crw/inep_saeb/saeb_diretor_unificada',
#   'prm_path': '/prm/usr/inep_saeb/KC2332_SAEB_DIRETOR_mapeamento_unificado_raw_V1.xlsx',
...
def find_repeated_org(parse_ba, sheet):
  count = {}
...
def fill_column(df, org, dst, _type, repeated):
...
def transform_df(df, parse_ba, sheet):
  ...
  repeated_org = find_repeated_org(parse_ba, sheet)
  
  for org, dst, _type in parse_ba[sheet]:
    repeated = org in repeated_org
    df = fill_column(df, org, dst, _type, repeated)
    
  df = df.drop(*repeated_org)
