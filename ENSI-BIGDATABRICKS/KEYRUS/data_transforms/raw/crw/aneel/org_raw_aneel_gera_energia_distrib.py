...
adl_lnd_path = "{adl_path}{lnd_path}".format(adl_path=var_adls_uri, lnd_path=lnd_path)
...
for sub_plan in var_prm_dict.values():
  for row in sub_plan:
    row[0] = __normalize_str(row[0]) if row[0].upper()!='N/A' else row[0]
...
df = spark.read.parquet(adl_lnd_path)
df = df.withColumn('POTENCIAINSTALADAKW', f.translate(f.col('POTENCIAINSTALADAKW'), ',' ,'.'))
