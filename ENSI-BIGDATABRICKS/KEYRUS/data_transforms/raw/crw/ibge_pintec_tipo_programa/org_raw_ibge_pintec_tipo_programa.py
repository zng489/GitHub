...
def __match_columns_quantity(df, columns_to_add):
...
for lnd_year_path in cf.list_subdirectory(dbutils, lnd_path):
  ...
  for i, file in enumerate(files_folder):
    ...
    if len(df.columns) != _df_n_columns:
      biggest_df = df if len(df.columns) > len(_df.columns) else __df
      smallest_df = df if len(df.columns) > len(_df.columns) else __df
      
      normalized_df = __match_columns_quantity(smallest_df, biggest_df.columns)
    else:
      biggest_df = _df
      smallest_df = df
    if _df:
