...
#   'prm_path': '/prm/usr/inep_inse/KC2332_INSE_socioeconomico_mapeamento_unificado_raw_V1.xlsx'
# }
...
for lnd_year_path in sorted(cf.list_subdirectory(dbutils, lnd_path), key=lambda p: p.split('/')[-1]):
    ...
    for org, dst, _type in var_prm_dict[year]:
      if org == 'N/A' and dst not in df.columns:
        ...
        df = (df
              ...
              .withColumn(dst, f.col(dst).cast(_type)))
    
    df = df.withColumn('NU_ANO', f.lit(year))
