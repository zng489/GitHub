...
for lnd_year_path in sorted(cf.list_subdirectory(dbutils, lnd_path), key=lambda p: int(p.split('/')[-1])):
  ...
  for i, file in enumerate(files_folder):
    ...
    sheet_name = file.split('/')[-1].replace('.parquet','')
    ...
    df = df.withColumn('TRIMESTRE', f.lit(sheet_name).cast('string'))
    df = df.withColumn('ANO', f.lit(year).cast('string'))
