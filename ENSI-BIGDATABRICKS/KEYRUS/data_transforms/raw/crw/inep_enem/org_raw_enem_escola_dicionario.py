...
# MAGIC Tabela/Arquivo Origem	\lnd\crw\inep_enem_escola__dicionario
# MAGIC Tabela/Arquivo Destino	\raw\crw\inep_enem\enem_escola_dicionario
# MAGIC </pre>
...
for col in df.columns:
  df = df.withColumn(col, f.when(f.col(col) == f.lit('None'), f.lit(None)).otherwise(f.col(col)))
...
df.coalesce(1).write.save(path=adl_sink, format="parquet", mode="overwrite")