...
"""
var_file = {'namespace':'unigest','file_folder':'kpi_escola_sesi_nivel_avancado','extension':'CSV','column_delimiter':';','encoding':'UTF-8','null_value':''}
...
.withColumn("cd_ano_fechamento_value_error", 
            ...
            .otherwise(0))\
.drop("split_file_name","file_year_month","file_year")
...
.withColumn("cd_ano_fechamento_value_error",
            ...
            .otherwise(f.lit(None)))\
.withColumn("errors", f.array(f.col("decimal_point_error"), f.col("numeric_error"), f.col("cd_ano_fechamento_value_error")))\
.drop("decimal_point_error", "numeric_error", "cd_ano_fechamento_value_error")
...
df.repartition(1).write.partitionBy('cd_ano_fechamento', 'dh_arq_in').parquet(path=var_sink, mode='overwrite')
