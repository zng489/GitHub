...
#  'schema': 'inep_ird',
#  'table': 'escolas',
#  'prm_path': '/tmp/dev/prm/usr/inep_ird/FIEC_inep_ird_escolas_mapeamento_unificado_trusted.xlsx'
# }

# adf = {
#  "adf_factory_name": "cnibigdatafactory",
#  "adf_pipeline_name": "org_ird_taxa_rendimento",
#  "adf_pipeline_run_id": "",
#  "adf_trigger_id": "",
#  "adf_trigger_name": "",
...
df = df.withColumn('CD_UF', f.when(f.col('CD_UF').isNull(), df['SG_UF']).otherwise(f.col('CD_UF')))
df = df.withColumn('CD_UF', f.col('CD_UF').cast("int"))

df = df.join(df_sg_uf, f.upper(df.CD_UF)==f.upper(df_sg_uf.CD_UF), how='left') \
       .drop(df.SG_UF).drop(df_sg_uf.CD_UF)
