...
src_tba, src_tbb,  src_tbc,  src_tbd,  src_tbe,  src_tbf,  src_tbg,  src_tbh, src_tbi, src_tbj, src_tbk, src_tbl = ["{}{}{}".format(var_adls_uri, var_dls["folders"]["raw"],t)  for t in var_tables["origins"]]
print(src_tba, src_tbb,  src_tbc,  src_tbd,  src_tbe,  src_tbf,  src_tbg,  src_tbh, src_tbi, src_tbj, src_tbk, src_tbl)
...
log_sessions = spark.read.parquet(src_tbl)
...
log_sessions = log_sessions.select(from_unixtime(col("session_start")).alias("data_inicio_sessao"), from_unixtime(col("session_end")).alias("data_fim_sessao"), col("user_id").alias("e_user_id"))

log_sessions = log_sessions.select("data_inicio_sessao", "data_fim_sessao", "e_user_id", year("data_inicio_sessao").alias('anosessao'), month("data_inicio_sessao").alias('messessao'), dayofmonth("data_inicio_sessao").alias('diamessessao'), dayofweek("data_inicio_sessao").alias('diasemanasessao'), date_format("data_inicio_sessao", "YYYYMM").alias("anomessessao"))

resultado = resultado.join(log_sessions, (resultado.user_id == log_sessions.e_user_id))

resultado = resultado.select ("ano", "nome", "email", "data_nascimento", 'sexo', "employment_date", "user_company", "id_iniciativa", "classification_name", "group_id", "group_name", "group_summary", "data_inicio", "data_termino", "user_id", "data_matricula", "cargahoraria1", "cargahoraria2", "cargahoraria3", "entidade", "ocupacao", "graduacao", "area", "anomatricula", 'mesmatricula', 'diamatricula', 'diasemanamatricula', "anomesmatricula", "quantidade", "UF", "data_avaliacao", "score", "completed", "data_inicio_sessao", "data_fim_sessao",  'anosessao', 'messessao', 'diamessessao', 'diasemanasessao', "anomessessao")
