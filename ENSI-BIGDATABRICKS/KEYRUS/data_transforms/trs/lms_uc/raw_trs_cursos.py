...
resultado = resultado.select("ano", "id_iniciativa", "classification_name", "group_id", "group_name", "group_summary", from_unixtime(col("group_open_date")).alias("data_inicio"), from_unixtime(col("group_close_date")).alias("data_termino"), "user_id", from_unixtime(col("addition_time")).alias("data_matricula"), "cargahoraria1", "cargahoraria2", col("field_3").alias("entidade"), col("field_44").alias("ocupacao"), col("field_46").alias("graduacao"), col("field_48").alias("area"))
...
users = users.select(col("user_id").alias("e_user_id"), "employment_date", "user_company")

resultado = resultado.join(users, (resultado.user_id == users.e_user_id))

resultado = resultado.select ("ano", "id_iniciativa", "classification_name", "group_id", "group_name", "group_summary", "data_inicio", "data_termino", "user_id", "employment_date", "user_company", "data_matricula", "cargahoraria1", "cargahoraria2", "cargahoraria3", "entidade", "ocupacao", "graduacao", "area", "anomatricula", 'mesmatricula', 'diamatricula', 'diasemanamatricula', "anomesmatricula", "quantidade", "UF", "data_avaliacao", "score", "completed")
