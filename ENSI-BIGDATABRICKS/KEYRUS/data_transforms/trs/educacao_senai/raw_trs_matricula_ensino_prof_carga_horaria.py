...
'''
var_tables =  {"origins": ["/bdo/bd_basi/rl_atendimento_metrica", "/bdo/bd_basi/tb_atendimento"], "destination": "/evt/matricula_ensino_prof_carga_horaria"}
...
# MAGIC 
# MAGIC Obter a maior data de referencia carregada #var_max_dh_ultima_atualizacao_oltp = SELECT MAX(dh_ultima_atualizacao_oltp) from matricula_ensino_prof_carga_horaria
# MAGIC 
...
# MAGIC 
# MAGIC INNER JOIN 
# MAGIC    -- Somente Matrículas em Ensino Profissional
# MAGIC    (SELECT DISTINCT cd_atendimento FROM tb_atendimento WHERE cd_tipo_atendimento = 1)
...
atendimento_metrica = atendimento_metrica.withColumn("cd_ano_mes_referencia", date_format(col("dt_ano_mes_referencia"), 'yyyyMM'))
...
atendimento = spark.read.parquet(source_atendimento).select(*atendimento_columns).filter(col("cd_tipo_atendimento")==1).dropDuplicates().drop("cd_tipo_atendimento")

new_data = atendimento_metrica.join(atendimento, ["cd_atendimento"], "inner")

# COMMAND ----------

# Replacing metric "106" to "105"
new_data = new_data.filter(col("cd_metrica_atendimento").isin(102,103,104,105,106))\
.withColumn("cd_metrica_atendimento", when(col("cd_metrica_atendimento") == 106, 105).otherwise(col("cd_metrica_atendimento")))
...
# MAGIC %md
# MAGIC <pre>
# MAGIC PASSO 5: Gera um registro único com todas as métricas relacionadas ao atendimento matricula prof carga horária
...
# MAGIC     Pivotear:
# MAGIC         CASE WHEN cd_metrica_atendimento = 102  THEN vl_valor_medido ELSE NULL END as qt_hr_aluno_hora
# MAGIC         CASE WHEN cd_metrica_atendimento = 103  THEN vl_valor_medido ELSE NULL END as qt_hr_estagio
# MAGIC         CASE WHEN cd_metrica_atendimento = 104  THEN vl_valor_medido ELSE NULL END as qt_hr_pratica_prof
# MAGIC         CASE WHEN cd_metrica_atendimento = 105  THEN vl_valor_medido ELSE NULL END as qt_hr_hora_aluno
# MAGIC 		Outros, Ignorar

# COMMAND ----------

when_otherwise_rules = {"qt_hr_aluno_hora": {"column": "cd_metrica_atendimento", "when": 102, "then": "vl_valor_medido", "otherwise":None}, 
                        "qt_hr_estagio": {"column": "cd_metrica_atendimento", "when": 103, "then": "vl_valor_medido", "otherwise":None}, 
                        "qt_hr_pratica_prof": {"column": "cd_metrica_atendimento", "when": 104, "then": "vl_valor_medido", "otherwise":None}, 
                        "qt_hr_hora_aluno": {"column": "cd_metrica_atendimento", "when": 105, "then": "vl_valor_medido", "otherwise":None},}
...
new_data = new_data.groupBy("cd_atendimento_matricula_ensino_prof", "cd_ano_mes_referencia", "dh_referencia", "dh_ultima_atualizacao_oltp", "dh_insercao_raw") \
                    .agg(sum("qt_hr_pratica_prof").alias("qt_hr_pratica_prof"), 
                         sum("qt_hr_hora_aluno").alias("qt_hr_hora_aluno"), 
                         sum("qt_hr_aluno_hora").alias("qt_hr_aluno_hora"), 
                         sum("qt_hr_estagio").alias("qt_hr_estagio"))
...
# MAGIC 
# MAGIC <pre>
# MAGIC  Chave de comparação de todas as coisas! ["cd_atendimento_matricula_ensino_prof", "cd_ano_mes_referencia"]
# MAGIC 
...
old_data_to_balance = old_data_active.join(new_data.select("cd_atendimento_matricula_ensino_prof", "cd_ano_mes_referencia").dropDuplicates(), ["cd_atendimento_matricula_ensino_prof", "cd_ano_mes_referencia"], "inner")
...
old_data_active_to_rewrite = old_data_active.join(new_data.select("cd_atendimento_matricula_ensino_prof", "cd_ano_mes_referencia").dropDuplicates(), ["cd_atendimento_matricula_ensino_prof", "cd_ano_mes_referencia"], "leftanti") 
...
for item in list_dh_insercao_raw:
  new_data = new_data.withColumn("lag_qt_hr_pratica_prof", lag("qt_hr_pratica_prof", 1).over(Window.partitionBy("cd_atendimento_matricula_ensino_prof", "cd_ano_mes_referencia").orderBy(asc("dh_ultima_atualizacao_oltp")))) \
                   .withColumn("lag_qt_hr_hora_aluno", lag("qt_hr_hora_aluno", 1).over(Window.partitionBy("cd_atendimento_matricula_ensino_prof", "cd_ano_mes_referencia").orderBy(asc("dh_ultima_atualizacao_oltp")))) \
                   .withColumn("lag_qt_hr_aluno_hora", lag("qt_hr_aluno_hora", 1).over(Window.partitionBy("cd_atendimento_matricula_ensino_prof", "cd_ano_mes_referencia").orderBy(asc("dh_ultima_atualizacao_oltp")))) \
                   .withColumn("lag_qt_hr_estagio", lag("qt_hr_estagio", 1).over(Window.partitionBy("cd_atendimento_matricula_ensino_prof", "cd_ano_mes_referencia").orderBy(asc("dh_ultima_atualizacao_oltp")))) 
  
  new_data = new_data.withColumn("qt_hr_pratica_prof", when((col("dh_insercao_raw") == item[0]) & (col("qt_hr_pratica_prof").isNull()), col("lag_qt_hr_pratica_prof")).otherwise(col("qt_hr_pratica_prof"))) \
                   .withColumn("qt_hr_hora_aluno", when((col("dh_insercao_raw") == item[0]) & (col("qt_hr_hora_aluno").isNull()), col("lag_qt_hr_hora_aluno")).otherwise(col("qt_hr_hora_aluno"))) \
                   ...
                   .withColumn("qt_hr_estagio", when((col("dh_insercao_raw") == item[0]) & (col("qt_hr_estagio").isNull()), col("lag_qt_hr_estagio")).otherwise(col("qt_hr_estagio"))) 
  
  new_data = new_data.drop("lag_qt_hr_pratica_prof", "lag_qt_hr_hora_aluno", "lag_qt_hr_aluno_hora", "lag_qt_hr_estagio")
  
  new_data = new_data.withColumn("qt_hr_pratica_prof", when((col("dh_insercao_raw") == item[0]) & (col("qt_hr_pratica_prof").isNull()), 0).otherwise(col("qt_hr_pratica_prof"))) \
                   .withColumn("qt_hr_hora_aluno", when((col("dh_insercao_raw") == item[0]) & (col("qt_hr_hora_aluno").isNull()), 0).otherwise(col("qt_hr_hora_aluno"))) \
                   .withColumn("qt_hr_aluno_hora", when((col("dh_insercao_raw") == item[0]) & (col("qt_hr_aluno_hora").isNull()), 0).otherwise(col("qt_hr_aluno_hora"))) \
                   .withColumn("qt_hr_estagio", when((col("dh_insercao_raw") == item[0]) & (col("qt_hr_estagio").isNull()), 0).otherwise(col("qt_hr_estagio"))) 
...
hash_columns = ["cd_atendimento_matricula_ensino_prof", "cd_ano_mes_referencia"] + qt_columns
new_data = new_data.withColumn("qt_hash", concat_ws("@", *hash_columns))

# COMMAND ----------

new_data = new_data.withColumn("lag_qt_hash", lag("qt_hash", 1).over(Window.partitionBy("cd_atendimento_matricula_ensino_prof", "cd_ano_mes_referencia").orderBy(asc("dh_referencia"))))
...
new_data = new_data.withColumn("row_number", row_number().over(Window.partitionBy("cd_atendimento_matricula_ensino_prof", "cd_ano_mes_referencia").orderBy(asc("dh_referencia"))))
...
new_data = new_data.withColumn("max_row_number", max("row_number").over(Window.partitionBy("cd_atendimento_matricula_ensino_prof", "cd_ano_mes_referencia")))
# Counting after, no records are lost

# COMMAND ----------

new_data = new_data.withColumn("lead_dh_ultima_atualizacao_oltp", lead("dh_ultima_atualizacao_oltp", 1).over(Window.partitionBy("cd_atendimento_matricula_ensino_prof", "cd_ano_mes_referencia").orderBy(asc("dh_ultima_atualizacao_oltp"))))
...
new_data = new_data.union(old_data_to_rewrite.select(new_data.columns)).orderBy(asc("cd_atendimento_matricula_ensino_prof"), asc("cd_ano_mes_referencia"), asc("dh_ultima_atualizacao_oltp"), asc("dh_referencia"))
