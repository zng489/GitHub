...
# MAGIC %md
# MAGIC 
# MAGIC # About Data Patch process:
# MAGIC - this notebook is very specific to the data patch correction process of the business "fta_gestao_financera" 
# MAGIC - this notebook will be used specifically for the following parameters:
# MAGIC   - year = 2020
# MAGIC   - month <= 8
...
var_user_parameters = {'closing': {'year': 2020, 'month': 4, 'dt_closing': '2020-11-12'}}
"""
...
# MAGIC ##### Documentation 
# MAGIC ```
# MAGIC Em função da Migração do Protheus do Oracle para MS-SQL, foram refeitas algumas cargas de arquivos que modificaram a data de atualização original e provocou diferença nos valores já divulgados para os meses retroativos de janeiro a agosto, a lógica abaixo é a ação de contorno para essa questão aplicável somente ao período de 2020-01 a 2020-08
# MAGIC 
# MAGIC IF  #prm_ano_fechamento = 2020 AND #prm_mes_fechamento <= 8:
# MAGIC 
# MAGIC ##### APPEND AO DATAFRAME CRIADO ####
# MAGIC    SELECT ...
# MAGIC    WHERE YEAR(dt_lancamento) = #prm_ano_fechamento 
# MAGIC    AND   MONTH(dt_lancamento) <= #prm_mes_fechamento 
# MAGIC    AND   dt_ultima_atualizacao_oltp > #prm_data_corte
# MAGIC    AND   dt_ultima_atualizacao_oltp <= '2020-11-12'
# MAGIC    AND
# MAGIC    ( (#prm_mes_fechamento = 1 AND cd_entidade_regional_erp_oltp IN    
# MAGIC      ('02AM0000','03AM0000','02BA0000','03BA0000','02CE0000','03CE0000','02DN0000','03DN0000','02RJ0000','03RJ0000')
# MAGIC      OR
# MAGIC      (#prm_mes_fechamento IN (2, 3) AND cd_entidade_regional_erp_oltp IN 
# MAGIC      ('02AM0000','03AM0000','02CE0000','03CE0000','02RJ0000','03RJ0000')
# MAGIC      OR
# MAGIC      (#prm_mes_fechamento >= 5 AND cd_entidade_regional_erp_oltp IN ('02AM0000','03AM0000','02RJ0000','03RJ0000')
# MAGIC    )
# MAGIC 
# MAGIC ##### DELETE NO DATAFRAME CRIADO ####
# MAGIC    IF #prm_ano_fechamento = 2020 AND #prm_mes_fechamento = 4
# MAGIC       DELETE dt_ultima_atualizacao_oltp > '2020-05-26 AND cd_entidade_regional_erp_oltp IN ('02PE0000','03PE0000')
# MAGIC ```
...
if var_parameters["prm_ano_fechamento"] == 2020 and var_parameters["prm_mes_fechamento"] <= 8:  
    
  if var_parameters["prm_mes_fechamento"] == 4:
    df = df_join.filter(
      (year(col("dt_lancamento")) == var_parameters["prm_ano_fechamento"]) 
      & (month(col("dt_lancamento")) <= var_parameters["prm_mes_fechamento"]) 
      & (col("dt_ultima_atualizacao_oltp") <= var_parameters["prm_data_corte"])
      & (~(col("cd_entidade_regional_erp_oltp").isin('02PE0000','03PE0000'))
         | ((col("dt_ultima_atualizacao_oltp") <= lit("2020-05-26").cast("date"))
            & (col("cd_entidade_regional_erp_oltp").isin('02PE0000','03PE0000')))) 
    ). \
    drop("dt_ultima_atualizacao_oltp", "dt_lancamento")
    ...
    df_to_append = df_join.filter(
      (year(col("dt_lancamento")) == var_parameters["prm_ano_fechamento"]) 
      & (month(col("dt_lancamento")) <= var_parameters["prm_mes_fechamento"]) 
      & (col("dt_ultima_atualizacao_oltp") > var_parameters["prm_data_corte"])
      & (col("dt_ultima_atualizacao_oltp") <= lit("2020-11-12").cast("date"))
      & (((lit(var_parameters["prm_mes_fechamento"]) == 1)
          & (col("cd_entidade_regional_erp_oltp").isin('02AM0000','03AM0000','02BA0000','03BA0000','02CE0000','03CE0000','02DN0000','03DN0000','02RJ0000','03RJ0000')))
         |((lit(var_parameters["prm_mes_fechamento"]).isin(2,3))
           & (col("cd_entidade_regional_erp_oltp").isin('02AM0000','03AM0000','02CE0000','03CE0000','02RJ0000','03RJ0000')))
         |((lit(var_parameters["prm_mes_fechamento"])>=5)
           & (col("cd_entidade_regional_erp_oltp").isin('02AM0000','03AM0000','02RJ0000','03RJ0000'))))
    ). \
    drop("dt_ultima_atualizacao_oltp", "dt_lancamento")
    
    df = df.union(df_to_append.select(df.columns))
